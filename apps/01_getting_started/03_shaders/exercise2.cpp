...
uniform vec3 horizontal_shift;

out vec3 vertex_color;

void main()
{
    gl_Position = vec4(a_pos + horizontal_shift, 1.0);
...
int main() {
    ...
    if (glGetUniformLocation(shader_program, "horizontal_shift") == -1) {
        std::cerr << "Failed to get uniform location for 'horizontal_shift'\n";
        glDeleteProgram(shader_program);
        glfwDestroyWindow(window);
        glfwTerminate();
        return EXIT_FAILURE;
    ...
    constexpr std::array<float, 18> vertices{
        // 位置坐标            // 顶点颜色
        -0.5F, -0.5F, 0.0F,  1.0F, 0.15F, 0.15F, 0.5F,  -0.5F, 0.0F,
        0.15F, 0.90F, 0.35F, 0.0F, 0.5F,  0.0F,  0.20F, 0.45F, 1.0F,
    };
    ...
    while (glfwWindowShouldClose(window) == GLFW_FALSE) {
        ...
        glUniform3f(glGetUniformLocation(shader_program, "horizontal_shift"), 0.5F, 0.0F, 0.0F);
