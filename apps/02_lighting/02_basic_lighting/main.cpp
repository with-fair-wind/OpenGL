/**
 * @file main.cpp
 * @brief 基础光照示例：环境光与漫反射。
 *
 * @details
 * 本示例开始使用顶点法线。片段着色器根据片段世界坐标、法线和光源位置，
 * 计算环境光 ambient 与漫反射 diffuse。法线描述表面朝向，是光照计算里最关键的输入之一。
 */
...
constexpr const char* window_title{"OpenGL Lab - Basic Lighting"};
...
int main() {
    ...
    while (glfwWindowShouldClose(window) == GLFW_FALSE) {
        ...
        glUniform3fv(glGetUniformLocation(object_program, "light_pos"), 1, glm::value_ptr(light_position));
