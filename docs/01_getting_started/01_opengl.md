...
早期的 OpenGL 使用**立即渲染模式**（Immediate Mode，也就是**固定渲染管线**），这个模式下绘制图形很方便。OpenGL 的大多数功能都被库隐藏起来，开发者很少有控制 OpenGL 如何进行计算的自由。而开发者迫切希望能有更多的灵活性。随着时间推移，规范越来越灵活，开发者对绘图细节有了更多的掌控。立即渲染模式确实容易使用和理解，但是效率太低。因此从 OpenGL 3.2 开始，规范文档开始废弃立即渲染模式，并鼓励开发者在 OpenGL 的**核心模式**（Core-profile）下进行开发，这个分支的规范彻底移除了所有旧的废弃功能。

当使用 OpenGL 的核心模式时，OpenGL 迫使我们使用现代的函数。当我们试图使用一个已废弃的函数时，OpenGL 会记录一个错误（例如 `GL_INVALID_OPERATION`）并忽略该调用，不会产生任何绘制效果。

> **订正：** 原英文教程写作 "OpenGL raises an error and stops drawing"（抛出一个错误并终止绘图），这个说法比较笼统。准确地说：核心模式规范**直接移除**了这些废弃函数，驱动不再保证提供它们；即使仍能调用，OpenGL 也只会记录错误并忽略该调用，而不会"终止"整个渲染流程。

现代函数的优势是更高的灵活性和效率，然而也更难于学习。立即渲染模式从 OpenGL**实际**运作中抽象掉了很多细节，因此它在易于学习的同时，也很难让人把握 OpenGL 具体是如何运作的。现代函数要求使用者真正理解 OpenGL 和图形编程，它有一些难度，然而提供了更多的灵活性、更高的效率，更重要的是可以更深入地理解图形编程。

这也是为什么我们的教程面向 OpenGL 3.3 的核心模式。虽然上手更困难，但这份努力是值得的。

现今，更高版本的 OpenGL 已经发布（写作时最新版本为 4.6），你可能会问：既然 OpenGL 4.6 都出来了，为什么我们还要学习 OpenGL 3.3？答案很简单：所有 OpenGL 的更高版本都是在 3.3 的基础上，引入了额外的功能，并没有改动核心架构。新版本只是引入了一些更有效率或更有用的方式去完成同样的功能。因此，所有的概念和技术在现代 OpenGL 版本里都保持一致。当你的经验足够时，你可以轻松使用来自更高版本 OpenGL 的新特性。
...
> **进阶（扩展的命名与转正）：** 扩展名的前缀透露了它的"出身"：
>
> - `GL_ARB_*` 由 Khronos 正式批准，`GL_EXT_*` 由多家厂商协商一致，`GL_NV_*`、`GL_AMD_*` 等是个别厂商的独家扩展。
> - 扩展足够流行后会被**吸收进核心规范**，函数去掉后缀成为正式 API，如 `GL_ARB_vertex_array_object` 的 `glGenVertexArrays` 后来进入了 OpenGL 3.0 核心。
> - 使用前必须先**查询**扩展是否可用（`glGetStringi(GL_EXTENSIONS, i)`），示例中的 `if (GL_ARB_extension_name)` 只是示意。
> - 本仓库使用 OpenGL 3.3 Core，教程涉及的函数几乎都是核心函数，无需手工处理扩展。
...
> **进阶（"设置"与"使用"为何分离）：** 这是为了**批量渲染**的效率：
>
> - 对驱动而言，切换状态代价高昂：改变混合/深度测试、切换着色器、重绑缓冲，都可能触发校验、管线重配置甚至着色器重编译。
> - 高效的做法是**少切换、多批量**：保持状态稳定，用同一组状态绘制尽量多的物体。
> - 立即渲染模式低效的根源正在于此：调用隐含大量细碎的状态切换，驱动难以优化；核心模式把状态显式化，切换频率由你掌控。
> - 本示例中 `glClearColor` 与 `glClear` 一"设置"一"使用"；后续 VBO/VAO 则是"绑定一次、绘制多次"。

## 对象

**一句话核心：对象是一组选项的集合，代表 OpenGL 状态的一个子集；通过"创建 → 绑定 → 设置 → 解绑"的工作流，你可以把多组设置都保存在驱动侧（缓冲等数据通常在 GPU 显存中），随时切换使用。**
...
> **进阶（名字即句柄与绑定语义）：** 对象 id 只是一个 `GLuint` 整数——**名字即句柄**，并不指向内存地址：
>
> - 对象的真实数据由驱动存放在"对象存储"中，程序只能通过名字间接引用；`glDelete*` 释放对象后，该名字可能被重新分配。
> - **绑定**把名字挂到上下文的目标位（target）上：同一缓冲可同时绑定到 `GL_ARRAY_BUFFER` 与 `GL_ELEMENT_ARRAY_BUFFER`，但每个目标位同一时刻只能有一个绑定。
> - 后续操作的都是"当前绑定"的对象：`glBufferData` 填的是当前绑在 `GL_ARRAY_BUFFER` 上的缓冲，这正是"先 bind 再设置"的由来。
> - 本节的 `glGenObject` 只是示意；下一节你将见到真实对象：VBO、VAO 与 `glGenBuffers`/`glBindBuffer`。
