...
- `glfwMakeContextCurrent(window)`：把窗口的 OpenGL 上下文设为**当前线程的当前上下文**（current context）。这里澄清一个常见的表述偏差：原教程（英文原文写作 "the main context of the current thread"）把它译为"当前线程的**主**上下文"——OpenGL/GLFW 中并没有"主上下文"这个概念，准确的说法是"**当前**上下文"：每个线程在任意时刻最多只有一个当前上下文，该线程上发出的所有 `gl*` 调用都作用于它。
...
> **进阶（多线程渲染）：** "当前上下文"是**线程局部**（thread-local）的概念，这正是多线程渲染的基石：
>
> - **每个线程**任意时刻最多只有一个当前上下文；**每个上下文**同一时刻也只能在一个线程上处于"当前"状态——把它设为另一线程的当前上下文，会先从原线程解绑。因此渲染指令只能由"上下文当前所在的那个线程"发出。
> - 常见的多线程分工：主线程持有上下文负责渲染，工作线程只做 CPU 侧的资源加载/计算（顶点数据、纹理解码等），再通过共享上下文或队列把数据交给渲染线程；不要同时在多个线程上操作同一个上下文。
> - 需要多线程渲染时，可以用 `glfwCreateWindow` 的 share 参数（本示例最后两个参数里的第二个）创建共享同一资源池（纹理、缓冲、着色器程序）的多个上下文，每个渲染线程各持一个。
> - 本示例是纯单线程程序：`main` 线程从调用 `glfwMakeContextCurrent` 到渲染循环结束全程持有当前上下文，不存在竞态问题。入门阶段记住"一个线程、一个当前上下文"这条铁律即可。
...
> **进阶（垂直同步与撕裂）：** 双缓冲消除了"画一半被看到"的闪烁，但交换本身并不等待显示器刷新：
>
> - 若 swap 恰好在显示器逐行扫描时发生，屏幕上会同时出现**两帧**的画面（上下半来自不同帧），这就是**撕裂**。
> - `glfwSwapInterval(1)` 开启垂直同步：swap 等到垂直消隐期才真正交换，撕裂消失，但帧率被锁为刷新率的约数、延迟略增；`glfwSwapInterval(0)` 关闭同步（基准测试常用）。
> - swap interval 是**每个上下文**的设置，须在 `glfwMakeContextCurrent` 之后调用才有效；本示例未显式设置，是否同步取决于平台默认值。
> - 以后遇到"画面撕裂"或"帧率被锁 60"，就能想到是这里的取舍。

### 输入：process_input

我们同样也希望能够在 GLFW 中实现一些输入控制，这可以通过使用 GLFW 的几个输入函数来完成。我们将会使用 GLFW 的 `glfwGetKey` 函数，它需要一个窗口以及一个按键作为输入。这个函数将会返回这个按键是否正在被按下。本仓库示例把输入逻辑放在匿名命名空间里的 `process_input` 函数中，让所有的输入代码保持整洁：
...
> **进阶（输入轮询 vs 事件回调）：** `glfwGetKey` 是**轮询**式输入——每帧主动问一次"这个键现在按下了吗"：
>
> - 轮询适合游戏循环：每帧依据"当前是否按住"决定移动/旋转，逻辑集中、时序确定；代价是必须每帧都调用。
> - 另一种方式是**回调**：用 `glfwSetKeyCallback` 注册函数，按键状态**变化时**（按下/释放/重复）由 GLFW 在 `glfwPollEvents` 期间调用，适合一次性动作（菜单、快捷键），但"持续按住"得自己维护状态。
> - 细节：`glfwGetKey` 只报告 `GLFW_PRESS`/`GLFW_RELEASE`；回调的 action 还能区分 `GLFW_REPEAT`（长按自动重复）。
> - 本示例用轮询实现 Esc 退出，正是"每帧处理输入"的标准写法；后续示例沿用。
...
注意，除了 `glClear` 之外，我们还调用了 `glClearColor` 来设置清空屏幕所用的颜色。当调用 `glClear` 函数清除颜色缓冲之后，整个颜色缓冲都会被填充为 `glClearColor` 里所设置的颜色。在这里，我们将屏幕设置为了深蓝灰色（值 `0.10, 0.14, 0.18`；最后一个分量是 alpha，1.0 表示完全不透明）。
