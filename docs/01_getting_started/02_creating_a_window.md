...
> **进阶（多窗口与离屏渲染）：** GLFW 并不限制只能创建一个窗口——`glfwCreateWindow` 可多次调用，每次返回独立的窗口与上下文：
>
> - 新窗口默认**可见**；创建不可见的离屏窗口，创建前加 `glfwWindowHint(GLFW_VISIBLE, GLFW_FALSE)`，配合 `glfwMakeContextCurrent` 切换当前上下文。
> - 多窗口可**共享资源**：`glfwCreateWindow` 的第五个参数（share）指向另一窗口，两个上下文共用纹理/缓冲/着色器资源池，但状态各自独立。
> - `glfwPollEvents` 调用一次即处理所有窗口的事件；渲染循环中对每个窗口分别 `glfwSwapBuffers`。
> - 本仓库示例都只创建一个可见窗口；理解"窗口/上下文可多实例"是后续离屏渲染（后处理、阴影贴图）的基础。
...
你可以看到代码非常复杂，而且很繁琐，我们需要对每个可能使用的函数都重复这个过程。幸运的是，有些库能简化此过程，其中 **GLAD** 是目前流行且保持更新的库。
