...
顶点数据经 **VBO** 上传显存，**VAO** 记录顶点属性布局，**EBO** 用索引复用顶点；顶点着色器输出 NDC 坐标，光栅化阶段把图元变成片段。这是「现代 OpenGL 核心管线」的第一次完整亮相。
...
纹理用图像代替逐顶点颜色，为物体提供细节。纹理坐标采样、环绕/过滤模式、多级渐远纹理（Mipmap）、纹理单元与多纹理混合，加上 stb_image 的加载流程，构成完整的贴图管线。
...
- **顶点缓冲对象（Vertex Buffer Object）**：调用显存并存储所有顶点数据供显卡使用的缓冲对象。
...
- **stb_image.h**：图像加载库。
- **纹理单元（Texture Units）**：通过绑定纹理到不同纹理单元，允许多个纹理在同一物体上渲染。
