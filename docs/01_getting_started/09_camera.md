...
- **右向量**（Right Vector，摄像机空间 x 轴正方向）：用上向量和方向向量做叉乘得到。两个向量叉乘的结果同时垂直于两者，于是得到指向 x 轴正方向的向量。
...
首先告诉 GLFW 隐藏并**捕捉**（Capture）光标：光标停留在窗口中、不显示，适合 FPS 摄像机。注册回调（`main()` 中 `glfwMakeContextCurrent` 之后）：
